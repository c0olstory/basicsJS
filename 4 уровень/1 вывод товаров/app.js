...
btn.forEach(function callback(button) {
...
function clickHandler(event) {
    ...
    let catg = event.target.dataset.type;
...
function showCategory(category) {
    let typeProduct = products[category];
    let blocksForDiv = "";

    typeProduct.forEach(function callback(product) {
        blocksForDiv += getProductMarkup(product);
    });

    prod.insertAdjacentHTML('afterbegin', blocksForDiv)
}
